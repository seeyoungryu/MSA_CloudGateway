...
/*
JWT 토큰은 서명(signing)과 검증(validation)이라는 두 가지 중요한 과정을 통해 보안을 유지합니다. 이미 다른 클래스에서 JWT 토큰을 생성한 후, 여기서는 그 토큰이 유효한지 확인하는 과정을 설명하고 있습니다. 이를 이해하려면 JWT의 구조와 서명/검증 과정에 대한 기본적인 이해가 필요합니다.
...
- **비밀 키 생성**: `Keys.hmacShaKeyFor(Decoders.BASE64URL.decode(secretKey))`를 통해 비밀 키를 생성합니다.
- **토큰 파싱 및 검증**: `Jwts.parser().verifyWith(key).build().parseSignedClaims(token)`을 통해 토큰을 파싱하고 서명을 검증합니다.
- **검증 결과 처리**: 토큰이 유효하면 `true`를 반환하고, 유효하지 않으면 예외를 던지고 `false`를 반환합니다.
